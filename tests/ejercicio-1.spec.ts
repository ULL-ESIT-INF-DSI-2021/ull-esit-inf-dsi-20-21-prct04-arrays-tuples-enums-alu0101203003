...
describe('Ejercicio 1', () => {
    it('decodeResistor(["Marron","Verde"]) returns value 15', () => {
      expect(decodeResistor(["Marron","Verde"])).to.be.deep.equal('15');
    });

    it('decodeResistor(["Marron","Verde","Violeta"]) returns value 15', () => {
      expect(decodeResistor(["Marron","Verde","Violeta"])).to.be.deep.equal('15');
    });
    it('decodeResistor(["Rojo","Negro"]) returns value 20', () => {
      expect(decodeResistor(["Rojo","Negro"])).to.be.deep.equal('20');
    });
});